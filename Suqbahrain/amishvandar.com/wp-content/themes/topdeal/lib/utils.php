...
function topdeal_content_product_detail(){
	...
	$right_span_md_class 	= topdeal_options()->getCpanelValue('sidebar_right_expand_md');
	$right_span_sm_class 	= topdeal_options()->getCpanelValue('sidebar_right_expand_sm');
	$sidebar_template 		= topdeal_options()->getCpanelValue('sidebar_product_detail');
	
	if( is_singular( 'product' ) ) :
		$sidebar_template = ( get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) != '' ) ? get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) : topdeal_options()->getCpanelValue('sidebar_product_detail');
		...
		<div class="modal-dialog block-popup-login">
			...
			<?php 
				if( class_exists( 'APSL_Lite_Class' ) ) :
					echo '<div class="login-line"><span>'. esc_html__( 'Or', 'topdeal' ) .'</span></div>';
	...
	if( !topdeal_mobile_check() ) : 
		...
		*/
		...
		$output .= '}(jQuery));';
			...
			'message' => esc_html__( 'Please enter your usename and password', 'topdeal' ),
		);
...
function post_init(){
	add_meta_box("topdeal_product_meta", esc_html__( 'Recommend Product:', 'topdeal' ), "topdeal_product_meta", "product", "side", "high");
...
?>
	...
	<p><label><b><?php esc_html_e( 'New Product', 'topdeal' ) ?></b></label> &nbsp;&nbsp;
		<input type="number" name="newproduct" value="<?php echo esc_attr( $newproduct ) ?>"/>
		<span class="p-description"><?php echo esc_html__( 'Set day for the new product label from the date publish product.', 'topdeal' ); ?></span>
	</p>
<?php }

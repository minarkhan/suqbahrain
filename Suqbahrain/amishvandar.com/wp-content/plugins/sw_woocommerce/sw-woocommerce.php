...
function sw_woocommerce_construct(){
	...
	add_action( 'wp_enqueue_scripts', 'sw_enqueue_script', 1 );
...
add_action( 'plugins_loaded', 'sw_woocommerce_construct', 30 );
...
function sw_enqueue_script(){	
	...
	wp_register_script( 'slick_slider', plugins_url( 'js/slick.min.js', __FILE__ ),array(), null, true );		
	$sw_text = array(
		'prev' => esc_html__( 'Prev', 'sw_woocommerce' ),
		'next' => esc_html__( 'Next', 'sw_woocommerce' )
	);
	wp_localize_script( 'slick_slider', 'sw_text', $sw_text );
	...
	$sw_countdown_text = array(
		...
		'hourl' 	 => esc_html__( 'Hours left', 'sw_woocommerce' ),
	);

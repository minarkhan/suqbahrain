...
<div class="container">
	<?php
		$topdeal_post_type = isset( $_GET['search_posttype'] ) ? $_GET['search_posttype'] : '';
		if( ( $topdeal_post_type != '' ) &&  locate_template( 'templates/search-' . $topdeal_post_type . '.php' ) ){
			get_template_part( 'templates/search', $topdeal_post_type );
		}else{ 
			if( have_posts() ){
		?>
			...
			<div id="post-<?php the_ID();?>" <?php post_class( 'theme-clearfix' ); ?>>
				<div class="entry clearfix">
					...
					<div class="entry-content">
						...
						<div class="bl_read_more"><a href="<?php the_permalink(); ?>"><?php esc_html_e('Read more','topdeal')?><i class="fa fa-angle-double-right"></i></a></div>

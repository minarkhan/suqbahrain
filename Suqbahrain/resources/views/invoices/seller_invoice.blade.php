			...
			<table class="padding text-left small border-bottom">
				<thead>
	                <tr class="gry-color" style="background: #eceff4;">
	                    <th width="35%">Seller Invoice Product Name</th>
	                ...
	                @foreach ($order->orderDetails->where('seller_id', $user_id) as $key => $orderDetail)
		                @if ($orderDetail->product)
							<tr class="">
                                ...
                                <td>{{ $mk = $orderDetail->seller->user->name }}
                                    @foreach ($mk as $seller)

                                    @endforeach
                                </td>

								{{-- <td class="gry-color">
                                    {{ $orderDetail->product->user->name }} <br/>
                                    {{ $orderDetail->seller_id }} <br/>
                                    {{ $orderDetail->product->user->name }} <br/>
                                    {{ $orderDetail->product->user->name }} <br/>
                                    {{ $orderDetail->product->user->name }} <br/>
                                </td> --}}

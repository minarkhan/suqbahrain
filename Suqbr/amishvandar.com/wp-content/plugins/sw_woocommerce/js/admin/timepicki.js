...
(function($) {
	...
	$.fn.timepicki = function(options) {
		...
		return this.each(function() {
			...
			if(settings.reset){
				$( "<a class='reset_time'>x</a>").insertAfter( ele );
			}
			ele_par.append(new_ele);
			var ele_next = $(this).parent().find(".timepicker_wrap");
	...
	$( '#woocommerce-product-data' ).on( 'click', '.sale_schedule', function() {
		...
		$wrap.find( '.sale_price_time_fields' ).show();
	...
	$( '#woocommerce-product-data' ).on( 'click', '.cancel_sale_schedule', function() {
		...
		$wrap.find( '.sale_price_time_fields' ).hide();
		$wrap.find( '.sale_price_time_fields' ).find( 'input' ).val('');

		return false;
	});
	
	$('.short_time').timepicki({
		show_meridian:false,
		min_hour_value:0,
		max_hour_value:23,
		step_size_minutes:1,

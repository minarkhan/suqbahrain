...
function topdeal_scripts() {	
	...
	$scheme = ( $scheme_meta != '' && $scheme_meta != 'none' && is_page() ) ? $scheme_meta : topdeal_options()->getCpanelValue('scheme');
	...
	$overflow_text = array(
		'more_text' => esc_html__( 'More...', 'topdeal' ),
		'more_menu'	=> topdeal_options()->getCpanelValue( 'more_menu' )
	);
	...
	*/
	if( topdeal_mobile_check() ) :
		...
		if( !topdeal_options()->getCpanelValue( 'mobile_jquery' ) ) :

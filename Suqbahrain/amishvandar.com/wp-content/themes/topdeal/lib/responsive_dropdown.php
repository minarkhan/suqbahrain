...
class Topdeal_Resmenu{
	...
	function Topdeal_MenuRes_AdFilter( $args ){
		...
		foreach( $topdeal_theme_locates as $topdeal_theme_locate ){
			if ( ( strcmp( $topdeal_theme_locate, $args['theme_location'] ) == 0 ) ) {	
	...
	function Topdeal_MenuRes_Filter( $args ){
		...
		foreach( $topdeal_theme_locates as $topdeal_theme_locate ){
			if ( ( strcmp( $topdeal_theme_locate, $args['theme_location'] ) == 0 ) ) {	
				...
				$args['items_wrap']	= '<button class="navbar-toggle" type="button" data-toggle="collapse" data-target="#ResMenu'. esc_attr( $topdeal_theme_locate ) .'">
					<span class="sr-only">Categories</span>
					<span class="icon-bar"></span>
					<span class="icon-bar"></span>
					<span class="icon-bar"></span>
				</button><div id="ResMenu'. esc_attr( $topdeal_theme_locate ) .'" class="collapse menu-responsive-wrapper"><ul id="%1$s" class="%2$s">%3$s</ul></div>';	
